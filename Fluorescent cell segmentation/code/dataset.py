...
# -----------------------------
def read_stack_from_folder(folder: str, expected_T: int = 190, exts=['.jpg','.png','.tif','.tiff']):
    """
    Read first expected_T frames from folder in sorted order.
    Returns numpy array (T,H,W) float32 normalized 0-1 (not normalized across dataset).
    """
    # collect files with acceptable extensions
    files = []
    for ext in exts:
        files += glob.glob(os.path.join(folder, f'*{ext}'))
    files = sorted(files)
    if len(files) < expected_T:
        raise RuntimeError(f"Folder {folder} has {len(files)} frames, expected at least {expected_T}.")
    files = files[:expected_T]
    frames = []
    for f in files:
        im = io.imread(f)
        if im.ndim == 3:
            # assume already grayscale? convert to gray to be safe
...
class StackDataset(Dataset):
    ...
    Args:
      ...
      expected_T: number of frames per stack (190)
    ...
    def __init__(self, stack_root_dirs: List[str], label_root: str = None,
                 patch_size: int = None, expected_T: int = 190, augment: bool = False):
    ...
    def __getitem__(self, idx):
        ...
        # read stack (T,H,W)
        stack = read_stack_from_folder(sd, expected_T=self.expected_T)  # float32
        # crop spatial region
...
class TestStackDataset(Dataset):
    """
    测试阶段用的数据集类
    每个样本对应一个 stack 文件夹（stack1, stack2, ...）
    返回 (1,T,H,W)
    """
    def __init__(self, test_root, expected_T=190):
        self.stack_dirs = sorted(glob.glob(os.path.join(test_root, "*")))
        self.expected_T = expected_T
    ...
    def __getitem__(self, idx):
        ...
        stack = read_stack_from_folder(stack_dir, expected_T=self.expected_T)  # (T,H,W)
        ...
        # 转为 (1,T,H,W)
        x = torch.from_numpy(stack).float().unsqueeze(0)
        return x, stack_name
...
def build_fold_loaders(train_root: str, train_label_root: str,
                       ...
                       expected_T: int = 190, num_workers: int = 4,
                       augment: bool = True,
    ...
    if mode == "full":
        ...
        train_ds = StackDataset(
            ...
            patch_size=patch_size, expected_T=expected_T, augment=augment
        )
        val_ds = StackDataset(
            val_dirs, label_root=train_label_root,
            patch_size=patch_size, expected_T=expected_T, augment=False
        )
